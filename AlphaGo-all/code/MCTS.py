from TreeNode import *
import copy
from AlphaBetaActionFilter import alpha_beta_search
from Board import Board
import numpy as np  # 导入 numpy
from save_game_record import query_personalized_records
from save_game_record import load_all_records, query_personalized_records

def softmax(x):
    probs = np.exp(x - np.max(x))
    probs /= np.sum(probs)
    return probs

class MCTS():
    """
    蒙特卡树搜索树
    """
    def __init__(self, policy_NN, factor=5, simulations=100, opponent_policy_NN=None, record_dir="models/personal_sgf"):
        self.root = TreeNode(None, 1.0)
        self.policy_NN = policy_NN
        self.opponent_policy_NN = opponent_policy_NN
        self.personal_records = load_all_records(record_dir)
        self.fator = factor
        self.simulations = simulations
        self.root_player = None  # 当前这轮搜索是谁的回合

    # def playout(self, state):
    #     """
    #     推演： 从根到叶进行推演，在叶上获取一个值，并通过其父级将其传播回来。
    #     """
    #     node = self.root
    #     while True:
    #         # 如果，是叶子节点就跳出
    #         if node.isLeaf():
    #             break
    #         # 否则
    #         #   就选择该节点所有孩子中《分数》最高的。
    #         #       action：落子动作
    #         #       node：孩子节点
    #         action, node = node.select(self.fator)
    #         state.do_move(action)
    #     # ---------------------------------------------------
    #     # 在原本的MCTS中，《评估》操作采用蒙特卡洛的方法，通过随机下棋
    #     # 模拟走完一次完整棋局 (称为 rollout), 得到胜负结果。模拟的结果反应在 leaf_value 变量中
    #     # ---------------------------------------------------
    #     # 根据当前的《状态(棋盘)》使用神经网络预测：下一步所有的动作以及
    #     # 对应的概率 + 此步未来的收益
    #     action_probs, leaf_value = self.policy_NN(state)

    #     # 检查一下当前《状态》是不是已经分出胜负
    #     gameOver, winner = state.gameIsOver()

    #     # 如果这盘棋还没结束
    #     if not gameOver:
    #         # 扩展当前节点
    #         node.expand(action_probs)
    #     else:
    #         # 平局
    #         if winner == -1:
    #             leaf_value = 0.0
    #         else:
    #             # 如果《模拟/预测》的结果，获胜的是当前的玩家，+1分 否则 -1分
    #             leaf_value = (
    #                 1.0 if winner == state.getCurrentPlayer() else -1.0
    #             )
        
    #     # 根据神经网络的预测结果 leaf_value
    #     # 自下而上《更新》叶子
    #     node.updateRecursive(-leaf_value)

    def playout(self, state, allowed_actions=None):
        node = self.root
        while True:
            if node.isLeaf():
                break
            # 只在allowed_actions中选择
            action, node = node.select(self.fator, allowed_actions)
            state.do_move(action)
        # ---------------------------------------------------
        # 在原本的MCTS中，《评估》操作采用蒙特卡洛的方法，通过随机下棋
        # 模拟走完一次完整棋局 (称为 rollout), 得到胜负结果。模拟的结果反应在 leaf_value 变量中
        # ---------------------------------------------------
        # 根据当前的《状态(棋盘)》使用神经网络预测：下一步所有的动作以及
        # 对应的概率 + 此步未来的收益
        #action_probs, leaf_value = self.policy_NN(state)

        # 检查一下当前《状态》是不是已经分出胜负
        gameOver, winner = state.gameIsOver()
        current_player = state.getCurrentPlayer()

        # 根据当前玩家决定使用哪一个策略网络
        if self.opponent_policy_NN is not None and current_player != self.root_player:
            action_probs, leaf_value = self.opponent_policy_NN(state)
            # 敌人的价值对我们是负的
            leaf_value = -leaf_value
        else:
            action_probs, leaf_value = self.policy_NN(state)

        # 如果这盘棋还没结束
        if not gameOver:
            node.expand(action_probs)
        else:
            if winner == -1:
                leaf_value = 0.0
            else:
                leaf_value = 1.0 if winner == self.root_player else -1.0
        
        # 根据神经网络的预测结果 leaf_value
        # 自下而上《更新》叶子
        node.updateRecursive(-leaf_value)


    # def getMoveProbs(self, state, flag_is_train):
    #     # print(str(self.root))
    #     """
    #     获取落子依据：按顺序进行所有推演，并返回可用操作及其相应的概率。
    #     """
    #     # state：当前游戏棋盘的状态。
    #     # 在（0，1）中控制探测(exploration)程序

    #     exploration = 1.0 if flag_is_train else 1e-3

    #     # 根据当前棋盘状态，经过 simulations 次数的模拟
    #     # 构建出了一个 MCTS 树，根节点是依托于当前棋盘。
    #     for _ in range(self.simulations):
    #         state_copy = copy.deepcopy(state)
    #         self.playout(state_copy)

    #     # 根据 MCTS 根节点，获取下一步落子的决策
    #     act_visits = [(act, node.N_visits) for act, node in self.root.children.items()]
    #     acts, visits = zip(*act_visits)
    #     act_probs = softmax(1.0/exploration * np.log(np.array(visits) + 1e-10))

    #     # 落子的位置 + 每一个位置的胜率
    #     return acts, act_probs

    def getMoveProbs(self, state, flag_is_train):
        # print(str(self.root))
        """
        获取落子依据：按顺序进行所有推演，并返回可用操作及其相应的概率。
        """
        # state：当前游戏棋盘的状态。
        # 在（0，1）中控制探测(exploration)程序

        exploration = 1.0 if flag_is_train else 1e-3

        self.root_player = state.getCurrentPlayer() #增加

        # 1. 提取棋盘状态
        list1, list2, list3, list_all = Board.extract_board_lists(state)

        if state.getCurrentPlayer() == 1:
        # 2. 剪枝过滤，获得候选动作（如前5个）
            candidate_moves = alpha_beta_search(list1, list2, list3, list_all, depth=2, ratio=1, top_n=5)
        else:
            candidate_moves = alpha_beta_search(list2, list1, list3, list_all, depth=2, ratio=1, top_n=5)

        # 3. MCTS模拟时只在候选动作中扩展
        for _ in range(self.simulations):
            state_copy = copy.deepcopy(state)
            self.playout(state_copy, allowed_actions=candidate_moves)

        # 4. 只统计候选动作的访问次数
        act_visits = [(act, node.N_visits) for act, node in self.root.children.items() if act in candidate_moves]
        if not act_visits:
            return [], []
        acts, visits = zip(*act_visits)
        act_probs = softmax(1.0/exploration * np.log(np.array(visits) + 1e-10))
        return acts, act_probs

    def updateMCTS(self, move):
        # 如果走的这一步在就是当前树根的孩子之一
        if move in self.root.children:
            # print('**********************************')
            # print('延续MCTS : ',str(self.root),'----->',str(self.root.children[move]))
            # 延续这棵树，更新树根
            self.root = self.root.children[move]
            self.root.father = None
        else:
            # 只有当整个对局分出胜负的时候，才重置整棵MCTS
            self.root = TreeNode(None, 1.0)
            # print('**********************************')
            # print('重新设置MCTS',str(self.root))

    def __str__(self):
        return "MCTS"