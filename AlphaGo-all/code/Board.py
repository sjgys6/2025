...
class Board(object):
    ...
    def extract_board_lists(state):
        ...
        width = state.width
        height = state.height
        list_all = [(i, j) for i in range(width) for j in range(height)]
        list1 = []  # 玩家1
        list2 = []  # 玩家2
        list3 = []  # 所有已落子点

        for pos, player in state.states.items():
            ...
            if player == 1:
                ...
                list2.append((x, y))
        return list1, list2, list3, list_all