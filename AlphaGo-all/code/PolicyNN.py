...
class PolicyValueNet(ResidualCNN):
    ...
    def sample_batch(self):
        ...
        按优先级采样一批数据
        """
        if not self.trainDataPool:
            return []
        priorities = [p for _, p in self.trainDataPool]
        total = sum(priorities)
        # 防止全为0
        if total == 0:
            probs = [1/len(priorities)] * len(priorities)
        else:
            probs = [p / total for p in priorities]
        batch = random.choices(self.trainDataPool, weights=probs, k=self.trainBatchSize)
        # 只取 play_data
    ...
    def memory(self, play_data, priority=1.0):
        """
        存储训练数据
        play_data: [(棋盘状态, 落子概率, 胜者预测), ..., ...]
        """
        ...
        for data in play_data:
            self.trainDataPool.append((data, priority))
